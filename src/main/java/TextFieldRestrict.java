...
public class TextFieldRestrict {
    ...
    public void onlyValidEmail (TextField textField){
        textField.textProperty().addListener(new ChangeListener<String>() { //Method let user write only valid e-mail
            @Override
            public void changed(ObservableValue<? extends String> observable, String oldValue,
                                String newValue) {
                if (!newValue.matches("[a-z\\d]+[\\w\\d.-]*@(?:[a-z\\d]+[a-z\\d-]+\\.){1,5}[a-z]{2,6}$")){
