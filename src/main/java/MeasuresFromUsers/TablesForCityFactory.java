...
public class TablesForCityFactory { //Klasa zawiera metody zwracjące lity konkretnych pomiarów dla wybranego miasta
    ...
    public ArrayList<TemperatureFromUser> showTemperaturesFromUserInCity(String city, ArrayList<TemperatureFromUser>
        ...
        for (int i = 0; i < temperaturesFromUserArrayList.size(); i++) {
            if (temperaturesFromUserArrayList.get(i).getCity().equals(city)) {
                result.add(new TemperatureFromUser(temperaturesFromUserArrayList.get(i).getDate(),
                        ...
                        temperaturesFromUserArrayList.get(i).getCity()));
            }
        }
        return result;
    }
